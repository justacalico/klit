...
# 12.0.0
- Made tab switches instant by using `CustomTransitionPage` with `Duration.zero` transition for all navigation routes, removing the full page transition animation when switching between tabs ([!75](https://gitlab.com/Openlyst/klit/-/merge_requests/75))
...
- Replaced remaining iOS-style bottom sheets (navbar more menu, post detail/fullscreen menu, settings picker sheets) with the standard popup menu used elsewhere in the app ([!41](https://gitlab.com/Openlyst/klit/-/merge_requests/41))
