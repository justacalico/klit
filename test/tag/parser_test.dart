 ...
 late Parser<List<TagNode>> parser;
  ...
  group('simple tags', () {
    test('parses a single tag', () {
      ...
      expect(nodes[0], const TagValue('tag1'));
    });
    ...
    test('parses multiple tags', () {
      ...
      expect(nodes[0], const TagValue('tag1'));
      expect(nodes[1], const TagValue('tag2'));
    });
  });
  ...
  group('namespaced tags', () {
    test('parses key:value', () {
      ...
      expect(nodes.length, 1);
      expect(nodes[0], const TagValue('key', 'value'));
    });

    test('parses mixed simple and namespaced', () {
      final nodes = parse('tag key:value');
      expect(nodes[0], const TagValue('tag'));
      expect(nodes[1], const TagValue('key', 'value'));
    });
  });
  ...
  group('groups', () {
    test('parses a group with children', () {
      ...
      expect(group.prefix, '');
      expect(group.children, [const TagValue('a'), const TagValue('b')]);
    });
  ...
  group('comments', () {
    test('parses a comment', () {
      ...
      expect(nodes.length, 1);
      expect(nodes[0], const TagComment('comment'));
    });

    test('parses a comment with spaces', () {
      final nodes = parse('#this is a comment');
      expect(nodes[0], const TagComment('this is a comment'));
    });
  });
  ...
  group('quoted values', () {
    test('parses a quoted value with spaces', () {
      ...
      expect(nodes[0], const TagValue('key', 'value with spaces'));
    });

    test('parses escaped quotes inside value', () {
      final nodes = parse(r'key:"value with \"quotes\""');
      expect(nodes[0], const TagValue('key', 'value with "quotes"'));
    });
  });
  ...
  group('mixed input', () {
    test('parses tags, group, namespaced, and comment together', () {
      ...
      expect(nodes[2], const TagValue('key', 'value'));
      expect(nodes[3], const TagComment('comment'));
    });
  });
